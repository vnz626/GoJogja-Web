...
@section('content')
    ...
    <section class="bg-gray-100 pt-4 pb-12">
        <div class="container mx-auto px-4">
            <div class="mb-8 flex flex-wrap border-b border-gray-200">
                @auth
                    ...
                    @foreach($blogs as $blog)
                        <article class="bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300 flex flex-col">
                            <a href="{{ route('blogs.show', $blog) }}">
                                @php
                                    $thumbnail = $blog->image_url ? asset('storage/' . $blog->image_url) : ($blog->images->first() ? asset('storage/' . $blog->images->first()->filename) : '/images/blog/default.jpg');
                                @endphp
                                <img src="{{ $thumbnail }}" alt="{{ $blog->title }}" class="w-full h-56 object-cover">
                            </a>
                            <div class="p-6 flex flex-col flex-grow">
                                <div class="mb-3">
                                    ...
                                    <span class="ml-2 text-xs text-gray-500">{{ $blog->created_at->format('d F Y') }}</span>
                                </div>
                                ...
                                <div class="mt-auto">
                                    <a href="{{ route('blogs.show', $blog) }}" class="inline-flex items-center text-custom-blue font-semibold hover:underline">
                                        Baca Selengkapnya
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
                                    </a>
                                </div>
                            </div>
                        </article>
